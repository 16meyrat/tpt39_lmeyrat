...
//--------------------------------------------------------------------
static size_t M=0, N=0, frames=0;// max 50000000
...
int OCL_initialize(size_t _M, size_t _N, size_t _frames){
  M = _M;
  N = _N;
	frames = _frames;
  
  printf("OpenCL configured to process %u * %u *%u videos\n", M, N, frames);
...
// Input buffers.
 ...
 output_buf = clCreateBuffer(context, CL_MEM_WRITE_ONLY,
     M*N*frames* sizeof(float), NULL, &status);
 ...
 checkError(status, "Failed to set argument 3");
	...
	printf("OpenCL configuration complete\n");
...
int OCL_Convolve(float* input, const float* kernel33, float* output)
{
    ...
    input_buf = clCreateBuffer(context, CL_MEM_READ_ONLY|CL_MEM_USE_HOST_PTR,
       M*N*frames * sizeof(float), input, &status);
    ...
    status = clEnqueueWriteBuffer(queue, kernel33_buf, CL_FALSE,
        0, 3*3* sizeof(float), kernel33,0, NULL, &write_event[0]);
    checkError(status, "Failed to transfer input B");
      

    const size_t global_work_size[] = {M, N, frames};
		const size_t work_group_size[] = {32, 8, 1};
    ...
    status = clEnqueueReadBuffer(queue, output_buf, CL_TRUE,
        0, M*N*frames* sizeof(float), output, 1, &kernel_event, &finish_event);
				checkError(status, "Failed to launch kernel");
	printf("debug\n");
